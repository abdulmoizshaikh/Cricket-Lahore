...
import {StyleSheet, View,Text,ImageBackground, KeyboardAvoidingView,ToastAndroid,TouchableOpacity} from 'react-native';
...
gettournament=()=>{
    ...
    console.log(team_id);
    var self=this;
 axios.post(global.url+"/api/joinedtournaments",{
     team_id:team_id,
...
console.log(this.state.team_id);
var self=this;
axios.post(global.url+"/api/droptournaments",{
 team_id:team_id,
        ...
        <View style={{flex:1}}>
         <Container  style={{paddingTop:23,backgroundColor:'#CBD8E9'}}>    
            <Header style={{backgroundColor:'#154360'}} >
                        ...
                        <Body>
                            <Text style={{color:'#fff',fontWeight:'bold',fontSize:25}}>Joined Tournaments</Text>
                        </Body>
                    </Header>                      
        
         <Content>
         
{this.state.flag ? this.state.joinedTrmnt.map(joinedTrmnt=>(
          <Card key={joinedTrmnt.id}>
              <CardItem style={{backgroundColor:"#fff"}}>
                <Body>
                  <Text style={{color:"black"}}>{joinedTrmnt.name}</Text>
                  <Text style={{color:"black"}}>{joinedTrmnt.startingDate}</Text>
                  <View style={{flex:1,alignItems:"center",justifyContent:"center"}}>
                            <Button 
                            style={{backgroundColor:'#FF0000'}} 
                            onPress={() => this.droptrmnt(joinedTrmnt.id)}
                            >
                            <Text style={{color: 'white',justifyContent:'center',alignItems:'center'}}>Drop</Text>
                            </Button>
                            <Button 
                            style={{backgroundColor:'#3C7DFE'}} 
                            onPress={() => this.tournamentID(joinedTrmnt.id)}
                            >
                            <Text style={{color: 'white',justifyContent:'center',alignItems:'center'}}>Matches</Text>
                            </Button>
                        </View>
                </Body>
              </CardItem>
