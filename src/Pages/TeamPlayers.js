...
import {StyleSheet, View,Text,Alert,ImageBackground, ToastAndroid,TouchableOpacity} from 'react-native';
...
teamsData=()=>{
    ...
    axios.post(global.url+"/api/getPlayerid",{
...
getteamPlayer=()=>{
    ...
    var self=this;
 axios.post(global.url+"/api/showPlayer",{
  team_id:team_id,
    }).then(function(response){
        //console.log("hi")
    console.log(response.data);
...
dropplayer=()=>{
    ...
    console.log(this.state.delID)
     var self=this;
  axios.post(global.url+"/api/dropplayer",{
        ...
        <View style={{flex:1}}>
         ...
         <ImageBackground source={myBackground} style={styles.backgroundImage}>    
            <Header style={{backgroundColor:'#154360'}} >
                        ...
                        <Body>
                            <Text style={{color:'#fff',fontWeight:'bold',fontSize:20}}>Team Players</Text>
                        </Body>
                    </Header> 
            ...
            <Card key={totalPlayers.id}>
                 <Card>
                    <CardItem style={{backgroundColor:'#154360'}}>
                        <Body>
                            <Text style={{color:"#fff",fontSize:18,fontWeight:'bold'}}>{this.Capitalize(totalPlayers.first_name)}</Text>
                        </Body>
                    </CardItem>
                </Card>
              <CardItem style={{backgroundColor:"#fff"}}>
                <Body>
                  <Text style={{color:"black",fontSize:15,fontWeight:'bold'}}><Text style={{color:"black",fontSize:15,fontWeight:'bold'}}>Email    : </Text>  {totalPlayers.email}</Text>
                  <Text style={{color:"black",fontSize:15,fontWeight:'bold'}}><Text style={{color:"black",fontSize:15,fontWeight:'bold'}}>Contact  : </Text>  {totalPlayers.contact}</Text>
                  {/* <Text style={{color:"black"}}>{totalPlayers.id}</Text> */}
           <Text> </Text>
<View key={totalPlayers.id} style={{flexDirection:'row',justifyContent:'center',alignItems:'center'}}>
<Button style={{backgroundColor:'#3C7DFE',width:80,justifyContent:'center',alignItems:'center'}} onPress={() => this.selectplayer(totalPlayers.id)}><Text style={{color:"#fff"}}>Select</Text></Button>
<Text>  </Text>
<Button style={{backgroundColor:'#FF0000',width:80,textAlign:'center',justifyContent:'center',alignItems:'center'}} 
                  onPress={()=>
                    this.setState({delID:totalPlayers.id.toString()},this._twoOptionAlertHandler())
                  }><Text style={{color:'#fff'}}>Drop</Text></Button>
</View>
