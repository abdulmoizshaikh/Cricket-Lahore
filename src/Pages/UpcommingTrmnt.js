...
import {StyleSheet, View,Text,ToastAndroid,ImageBackground, KeyboardAvoidingView,TouchableOpacity} from 'react-native';
...
selectplayer=(trmntid,requiredTeam)=>{
    ...
    var self=this;
 axios.post(global.url+"/api/jointournament",{
        ...
        <View style={{flex:1}}>
         <Container  style={{paddingTop:23,backgroundColor:'#CBD8E9'}}>    
            <Header style={{backgroundColor:'#154360'}} >
                        ...
                        <Body>
                            <Text style={{color:'#fff',fontWeight:'bold',fontSize:25}}>Upcomming Tournamentt</Text>
                        </Body>
                    </Header> 

             <Content>

                {this.state.isVisible ? this.state.trmntTeams.map(trmntTeams=>(
                    <Card key={trmntTeams.id}>
                        <CardItem style={{backgroundColor:"#fff"}}>
                            <Body>
                                <Text style={{color:"black",fontSize:14,fontWeight:"bold"}}><Text>Team Name : </Text>{trmntTeams.name}</Text>
                                <Text style={{color:"black",fontSize:14,fontWeight:"bold"}}><Text>Match Vanue : </Text>{trmntTeams.venue}</Text>
                                <Text style={{color:"black",fontSize:14,fontWeight:"bold"}}><Text>Start Date : </Text>{trmntTeams.startingDate}</Text>
                                <Text style={{color:"black",fontSize:14,fontWeight:"bold"}}><Text>Ending Date :</Text>{trmntTeams.endingDate}</Text>
                   <View style={styles.container}>
                            <Button
                            style={{backgroundColor:'#3C7DFE',width:80,justifyContent:'center',alignItems:'center'}} 
                            onPress={() => this.selectplayer(trmntTeams.id,trmntTeams.requiredTeam)}
                            >
                            <Text style={{color: 'white',justifyContent:'center',alignItems:'center'}}>Join</Text>
                            </Button>
